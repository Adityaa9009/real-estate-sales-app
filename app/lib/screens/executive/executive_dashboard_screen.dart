...
class _ExecutiveDashboardScreenState extends State<ExecutiveDashboardScreen> {
  int _selectedSidebarIndex =
      0; // 0: Add Emp, 1: Add Cust, 2: All Emp, 3: All Cust / Assign
                ...
                ListTile(
                  ...
                  onTap: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Executive system settings configured.'),
                      ),
                    );
                  },
                ),
  ...
  Future<void> _saveCustomer() async {
    ...
    try {
      await DatabaseService.addCustomer(
        ...
        email: _custEmailCtrl.text.trim().isEmpty
            ? null
            : _custEmailCtrl.text.trim(),
      );
      _custNameCtrl.clear();
            ...
            children: [
              Expanded(
                child: SearchBarWidget(
                  ...
                  onChanged: (val) =>
                      setState(() => _empSearch = val.toLowerCase()),
                ),
              ),
              const SizedBox(width: 12),
              ChoiceChip(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                        'Role: ${e.role.label} • ${e.active ? "Active" : "Inactive"}',
                        style: const TextStyle(
                          fontSize: 12,
                          color: AppColors.textSecondary,
                        ),
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  'Phone: ${cust.maskedPhone} • Status: ${cust.status.label}',
                  style: const TextStyle(color: AppColors.textSecondary),
                ),
                trailing: cust.assignedInsideSalesName != null
                    ? Chip(
                        label: Text(
                          'Assigned: ${cust.assignedInsideSalesName}',
                        ),
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: StreamBuilder<List<Employee>>(
          ...
          builder: (context, snapshot) {
            ...
            if (staff.isEmpty) {
              return const Text('No inside sales staff available.');
            }
            return Column(
